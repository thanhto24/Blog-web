...
const PostSearchResult = () => {
  ...
  return (
    <div>
      ...
      <div className="mx-auto mt-10 mb-10 min-h-screen min-w-max max-w-4xl rounded-lg bg-gray-400 p-4 shadow-md">
          ...
          {safePostList ? (
            <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-2">
              {safePostList.map((post) => (
                <div
                  key={post._id}
                  className="transform-origin-center mx-auto h-full min-h-[150px] w-96 transform rounded-lg border border-gray-300 bg-white p-4 transition-transform hover:scale-105 hover:shadow-lg"
                >
