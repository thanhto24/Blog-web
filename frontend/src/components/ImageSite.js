...
const ImageSite = ({ refreshImages }) => {
  ...
  const fetchImages = async () => {
    try {
      const response = await fetch(`https://blog-web-be-a7k7.onrender.com/images?image_owner=${userEmail}`, {
        method: 'GET',
  ...
  const handleCopyUrl = (id) => {
    const url = `https://blog-web-be-a7k7.onrender.com/images/${id}`;
    navigator.clipboard
      .writeText(url)
      .then(() => {
      ...
      <div className="grid grid-cols-1 gap-4">
        {images.length > 0 ? (
          images.map((image) => (
            ...
            >
              <img
                src={`https://blog-web-be-a7k7.onrender.com/images/${image._id}`}
