...
const Header = () => {
  ...
  const handleGoogleLogin = () => {
    window.location.href = 'https://blog-web-be.vercel.app/auth/google'; // Redirect to the backend route for Google login
  };
  ...
  const decodedToken = async (token) => {
    try {
      const response = await fetch('https://blog-web-be.vercel.app/auth/decodeToken', {
        method: 'POST',
