...
const googleCallback = (req, res) => {
  passport.authenticate('google', async (err, user) => {
    if (err || !user) {
      return res.redirect('https://blog-web-aiyr.onrender.com'); // Redirect to frontend login on failure
    }
    ...
    try {
      ...
      res.redirect(`https://blog-web-aiyr.onrender.com?token=${token}`);
    } catch (error) {
      console.error('Error generating token:', error);
      res.redirect('https://blog-web-aiyr.onrender.com'); // Handle error and redirect
    }
  })(req, res);
};
...
const facebookCallback = (req, res) => {
  passport.authenticate('facebook', (err, user) => {
    if (err || !user) {
      return res.redirect('https://blog-web-aiyr.onrender.com/login'); // Redirect to frontend login on failure
    }

    // Successful login, send user back to frontend with user data or token
    const token = 'some-jwt-token'; // Generate a token if needed
    res.redirect(`https://blog-web-aiyr.onrender.com/dashboard?token=${token}`);
  })(req, res);
};
